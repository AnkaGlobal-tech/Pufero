-- Neutral brand naming (multi-brand fork prep): tier tags tier-* instead of anka-tier-*

UPDATE tiers SET shopify_customer_tag = 'tier-bronze'
WHERE shopify_customer_tag = 'anka-tier-bronze';

UPDATE tiers SET shopify_customer_tag = 'tier-silver'
WHERE shopify_customer_tag = 'anka-tier-silver';

UPDATE tiers SET shopify_customer_tag = 'tier-gold'
WHERE shopify_customer_tag = 'anka-tier-gold';

UPDATE tiers SET shopify_customer_tag = 'tier-certified-pro'
WHERE shopify_customer_tag = 'anka-tier-certified-pro';
...
BEGIN
  ...
  VALUES
    (p_store_id, 'bronze', 'Bronze', 0, 0, 1, 'tier-bronze', 1),
    (p_store_id, 'silver', 'Silver', 500, 5, 1.25, 'tier-silver', 2),
    (p_store_id, 'gold', 'Gold', 1500, 10, 1.5, 'tier-gold', 3),
    (p_store_id, 'certified_pro', 'Certified Pro', 5000, 15, 2, 'tier-certified-pro', 4)
