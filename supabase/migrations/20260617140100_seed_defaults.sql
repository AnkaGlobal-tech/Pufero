-- Default store configuration + dev store seed
...
INSERT INTO stores (shop_domain, name, installed_at)
VALUES ('anka-loyalt-dev.myshopify.com', 'Anka Loyalty Dev', now())
ON CONFLICT (shop_domain) DO UPDATE
  SET name = EXCLUDED.name,
      installed_at = COALESCE(stores.installed_at, EXCLUDED.installed_at);

SELECT seed_store_defaults(id)
FROM stores
WHERE shop_domain = 'anka-loyalt-dev.myshopify.com';
